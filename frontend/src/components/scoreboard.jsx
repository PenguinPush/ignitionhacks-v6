...
export default function BadmintonScoreboard() {
    ...
    <div className={`relative bg-card border border-border rounded-2xl shadow-lg p-6 ${isFullscreen ? "w-3/4 h-3/4 flex flex-col items-center justify-center" : ""}`}>
      ...
      <div className={`grid grid-cols-2 gap-8 mb-8 ${isFullscreen ? "gap-16" : ""}`}>
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div
            className={`relative perspective ${flapSize}`}
          >
            <div
              className={`absolute w-full h-full rounded-lg bg-red-600 text-white font-mono font-extrabold flex items-center justify-center backface-hidden transform transition-transform duration-300 ${
                flipP1 ? "rotateX-180" : ""
              }`}
            >
              {player1Score.toString().padStart(2, "0")}
            </div>
            <div className={`absolute w-full h-full rounded-lg bg-red-500 text-white font-mono font-extrabold flex items-center justify-center rotateX-180 backface-hidden`}>
            {(player1Score + 1).toString().padStart(2, "0")}
            </div>
          </div>
          <button
            onClick={addPointP1}
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            <div
              className={`absolute w-full h-full rounded-lg bg-blue-600 text-white font-mono font-extrabold flex items-center justify-center backface-hidden transform transition-transform duration-300 ${
                flipP2 ? "rotateX-180" : ""
              }`}
            >
              {player2Score.toString().padStart(2, "0")}
            </div>
            <div className={`absolute w-full h-full rounded-lg bg-blue-500 text-white font-mono font-extrabold flex items-center justify-center rotateX-180 backface-hidden`}>
            {(player2Score + 1).toString().padStart(2, "0")}
            </div>
          </div>
          <button
            onClick={addPointP2}
      ...
      {winner && (
          <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-50">
            ...
            <button
              onClick={resetGame}
              className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-2xl"
            >
              Play Again
            </button>
          </div>
  ...
  >
    {isFullscreen ? (
      <Minimize className="w-6 h-6 text-primary" />
    ) : (
